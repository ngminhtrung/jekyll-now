...
function RadarChart(id, data, options) {
  var config = {
    ...
    maxDataValue: 0, //What is the value that the biggest circle will represent
  ...
  var axisList = data[0].map(function(i, j) {
    ...
    axisCount = axisList.length, //The number of different axes
    radiusLargestCircle = Math.min(config.w / 2, config.h / 2), //Radius of the outermost circle
    Format = d3.format("%"), //Percentage formatting
    angleSlice = Math.PI * 2 / axisCount; //The width in radians of each "slice"
  ...
  axisGrid
    ...
    .attr("fill", "#737373")
    .text(function(d, i) {
      return Format(maxDataValue * d / config.levels);
    });
  ...
  axis
    ...
    .attr("x2", function(d, i) {
      return (
        rScale(maxDataValue * 1.1) * Math.cos(angleSlice * i - Math.PI / 2)
      );
    })
    .attr("y2", function(d, i) {
      return (
        rScale(maxDataValue * 1.1) * Math.sin(angleSlice * i - Math.PI / 2)
      );
    })
    .attr("class", "line")
    .style("stroke", "white")
